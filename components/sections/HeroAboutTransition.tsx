...
export default function HeroAboutTransition() {
  ...
  const heroOpacity = useTransform(p, [0.28, 0.56], [1, 0]);
  const heroY = useTransform(p, [0.22, 0.56], [0, -48]);
  const heroScale = useTransform(p, [0.22, 0.52], [1, 0.97]);

  // Left black panel: slides left during breakup
  const heroLeftX = useTransform(p, [0.18, 0.44], [0, -32]);
  const heroLeftOpacity = useTransform(p, [0.24, 0.5], [1, 0]);

  // Right blue/portrait panel: slides right + fades
  const heroRightX = useTransform(p, [0.18, 0.44], [0, 40]);
  const heroRightOpacity = useTransform(p, [0.24, 0.5], [1, 0]);

  // Stats row: compresses down and fades
  const statsY = useTransform(p, [0.14, 0.36], [0, 28]);
  const statsOpacity = useTransform(p, [0.14, 0.34], [1, 0]);
  const statsScaleY = useTransform(p, [0.14, 0.34], [1, 0.6]);

  // Marquee (orange bottom bar): fades during phase 1
  const marqueeOpacity = useTransform(p, [0.08, 0.22], [1, 0]);

  // Hero headline words: subtle parallax drift
  const backendY = useTransform(p, [0.0, 0.28], [0, -16]);
  const withX = useTransform(p, [0.14, 0.36], [0, -24]);
  const tasteY = useTransform(p, [0.14, 0.36], [0, 22]);

  // Portrait frame: rises and fades
  const portraitY = useTransform(p, [0.1, 0.42], [0, -36]);
  const portraitOpacity = useTransform(p, [0.2, 0.46], [1, 0]);

  // Badge: drifts & shrinks
  const badgeRotate = useTransform(p, [0.0, 0.28], [-4, -10]);
  const badgeOpacity = useTransform(p, [0.18, 0.38], [1, 0]);
  const badgeScale = useTransform(p, [0.18, 0.38], [1, 0.7]);

  // ── ABOUT LAYER ────────────────────────────────────────────────────────────

  // Shell border frame fades in early
  const shellOpacity = useTransform(p, [0.3, 0.46], [0, 1]);

  // "I BUILD / BREAK / FIX" — emerges from left panel position
  const buildY = useTransform(p, [0.38, 0.62], [72, 0]);
  const buildOpacity = useTransform(p, [0.38, 0.6], [0, 1]);
  const buildScale = useTransform(p, [0.38, 0.62], [0.94, 1]);
  ...
  const bioOpacity = useTransform(p, [0.44, 0.62], [0, 1]);
  const bioY = useTransform(p, [0.44, 0.64], [20, 0]);

  // Philosophy card — PAPER UNCRUMPLE (phase 3 centrepiece)
  // Starts crumpled: skewed clip-path + rotation + blur + scale
  const paperOpacity = useTransform(p, [0.42, 0.52], [0, 1]);
  ...
  const paperSkewX = useTransform(p, [0.42, 0.52, 0.64, 0.72], [-8, -3, 1, 0]);
  const paperSkewY = useTransform(p, [0.42, 0.52, 0.64, 0.72], [4, 2, -0.5, 0]);
  ...
  const backendAboutY = useTransform(p, [0.52, 0.72], [52, 0]);
  const backendAboutOpacity = useTransform(p, [0.52, 0.7], [0, 1]);
  const backendAboutScale = useTransform(p, [0.52, 0.72], [0.95, 1]);

  // Hardware card — slides in from left
  const hardwareX = useTransform(p, [0.56, 0.74], [-64, 0]);
  const hardwareOpacity = useTransform(p, [0.56, 0.72], [0, 1]);

  // Interests strip — rises from bottom
  const interestsY = useTransform(p, [0.6, 0.78], [48, 0]);
  const interestsOpacity = useTransform(p, [0.6, 0.76], [0, 1]);
  const interestsScale = useTransform(p, [0.6, 0.78], [0.96, 1]);
  ...
  const builtOpacity = useTransform(p, [0.64, 0.78], [0, 1]);

  // Footer — rises last
  const footerY = useTransform(p, [0.72, 0.88], [32, 0]);
  const footerOpacity = useTransform(p, [0.72, 0.86], [0, 1]);
  ...
  return (
    <>
      ...
      <section
        ref={stageRef}
        className="relative hidden lg:block bg-[#EEE7DC]"
        style={{
      ...
      >
        ...
        <div
          className="sticky top-0 overflow-hidden bg-[#EEE7DC]"
          style={{ height: "100svh" }}
        >
