...
function PaperClip({ side = "top" }: { side?: "top" | "left" }) {
  if (side === "left") {
    return (
      <div className="absolute -left-[18px] top-10 z-20">
        <svg width="18" height="52" viewBox="0 0 18 52" fill="none">
          <path
            ...
            stroke="#888" strokeWidth="2.5" strokeLinecap="round" fill="none"
          />
        </svg>
      </div>
    );
  ...
  return (
    <div className="absolute -top-[22px] left-8 z-20">
...
function CodeStamp() {
  return (
    ...
    >
      <span style={{ fontFamily: "monospace", fontSize: 18, fontWeight: 900, lineHeight: 1 }}>&lt;/&gt;</span>
      <span style={{ fontFamily: "monospace", fontSize: 7, fontWeight: 700, letterSpacing: "0.08em", marginTop: 2 }}>OPEN SOURCE</span>
    </div>
  );
}
  ...
  return (
    ...
    >
      <svg width="28" height="28" viewBox="0 0 32 32" fill="none">
        <ellipse cx="16" cy="10" rx="10" ry="4" stroke="#4a6a8a" strokeWidth="2" fill="none"/>
        <path d="M6 10v6c0 2.2 4.5 4 10 4s10-1.8 10-4v-6" stroke="#4a6a8a" strokeWidth="2" fill="none"/>
        <path d="M6 16v6c0 2.2 4.5 4 10 4s10-1.8 10-4v-6" stroke="#4a6a8a" strokeWidth="2" fill="none"/>
      </svg>
      <span style={{ fontFamily: "monospace", fontSize: 7, fontWeight: 700, letterSpacing: "0.07em", marginTop: 3 }}>RESEARCH</span>
      <span style={{ fontFamily: "monospace", fontSize: 7, fontWeight: 700, letterSpacing: "0.07em" }}>SYSTEMS</span>
    </div>
  );
}
...
function AccessBadge({ label }: { label: string }) {
  ...
  return (
    ...
    >
      {lines.map((l, i) => <div key={i}>{l.trim()}</div>)}
    </div>
  );
}
...
// Barcode decoration
function Barcode() {
  const bars = [3,1,2,1,3,2,1,2,1,3,1,2,3,1,2,1,1,3,2,1,3,1,2];
  return (
    <div className="flex items-end gap-[1.5px]" style={{ height: 28 }}>
      {bars.map((w, i) => (
        <div key={i} style={{ width: w, height: `${60 + (i % 3) * 13}%`, background: "#555" }} />
      ))}
    </div>
  );
}
...
export default function ExperienceCard({
  ...
  return (
    <motion.div
      ...
      initial={{ opacity: 0, y: 40, rotate: entry.rotate - 3 }}
    ...
    >
      ...
      <div
        style={{
          ...
          padding: isContributor ? "24px 22px 20px 28px" : "24px 22px 22px 24px",
          width: "100%",
          position: "relative",
          overflow: "hidden",
        }}
      >
        ...
        <div
          ...
          style={{
            width: 52,
            height: "100%",
            background: "repeating-linear-gradient(135deg, transparent, transparent 4px, rgba(232,66,10,0.18) 4px, rgba(232,66,10,0.18) 8px)",
            pointerEvents: "none",
          }}
        />
        ...
        {/* Divider */}
        <div style={{ width: "100%", height: 2, background: isContributor ? "#333" : "#ccc", marginBottom: 12 }} />

        {/* Bullets */}
        <ul className="flex flex-col" style={{ gap: 6, marginBottom: 14 }}>
          {entry.bullets.map((b, i) => (
            <li key={i} className="flex items-start gap-2">
              <span style={{ color: "#E8420A", fontWeight: 900, fontFamily: "monospace", fontSize: 13, flexShrink: 0, marginTop: 1 }}>{">"}</span>
              <span
                style={{
                  fontFamily: "var(--font-archivo), sans-serif",
                  fontSize: 13,
        ...
        {isContributor && (
          <div style={{ marginBottom: 14 }}>
            <div className="flex items-center gap-2" style={{ marginBottom: 4 }}>
              <span style={{ fontFamily: "monospace", fontSize: 11, fontWeight: 700, color: "#888", letterSpacing: "0.08em" }}>STATUS:</span>
              <span style={{ fontFamily: "monospace", fontSize: 11, fontWeight: 700, color: "#CFDE00", letterSpacing: "0.08em" }}>CONTRIBUTING</span>
              <span style={{ fontFamily: "monospace", fontSize: 11, fontWeight: 700, color: "#888", marginLeft: "auto" }}>100%</span>
            </div>
            <ProgressBar value={100} />
            <div className="flex items-center gap-2" style={{ marginTop: 4 }}>
              <span style={{ fontFamily: "monospace", fontSize: 10, fontWeight: 700, color: "#888", letterSpacing: "0.06em" }}>LEVEL</span>
              <ProgressBar value={80} />
            </div>
            <div className="flex items-center gap-2" style={{ marginTop: 4 }}>
              <span style={{ fontFamily: "monospace", fontSize: 10, fontWeight: 700, color: "#888", letterSpacing: "0.06em" }}>DEVEL:</span>
        ...
        {/* Tags */}
        <div className="flex flex-wrap gap-[6px]" style={{ marginBottom: 14 }}>
          {entry.tags.map((tag) => (
            <span
              key={tag}
              style={{
        ...
        <div className="flex items-end justify-between" style={{ marginTop: 4 }}>
          <div>
            ...
            <div style={{ fontFamily: "monospace", fontSize: 9, color: isContributor ? "#666" : "#888", letterSpacing: "0.08em" }}>
        ...
        {/* Stamps */}
        {entry.stamp === "code" && <CodeStamp />}
        {entry.stamp === "research" && <ResearchStamp />}
      </div>
    </motion.div>
  );
}
